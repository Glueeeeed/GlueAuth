    ...
    try {
        ...
        if (encryptedSecret != null) {
            try {
                ...
                const proof: any = await generateProofs(merkleRoot, sessionData.sessionID, secret, uuid);
...
async function getMerkleRoot() : Promise<Group> {
    const response: Response = await fetch('/gluecrypt/api/zkp/members', {
        method: 'GET',
        headers: {
...
async function generateProofs(group : Group, sessionID: string, privateKey: string, uuid: string) {
    try {
        ...
        console.log(e);
        showError("Nie udało sie udowodnić że ta tożsamość należy do ciebie. Dzieje się tak gdy tożsamość nie istnieje lub nie jest twoja");
...
async function authenticateViaProof(proof : object , sessionID : string,): Promise<void> {
    try {
        const authResponse = await fetch('/gluecrypt/api/auth/proof', {
            method: 'POST',
            headers: {
                Accept: 'application/json',
